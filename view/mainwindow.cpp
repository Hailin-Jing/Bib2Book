...
int MainWindow::parse_UpdateJSON(QString str, bool iniCheck)
{
    ...
    if(root_Doc.isObject())
    {
        ...
        QJsonObject PulseValue = root_Obj.value("Latest").toObject();
        ...
        {
            QString warningStr =  tr("New version detected!\nVersion number:") + Verison + "\n" + tr("Update time: ") + UpdateTime + "\n" + tr("Release Notes: \n") + ReleaseNote;
            int ret = QMessageBox::warning(this, tr("Check for update"),  warningStr, tr("Go to download"), tr("Do not download"));
            if(ret == 0)    //点击更新
...
void MainWindow::on_actionUpdate_triggered()
{
    ...
    quest.setUrl(QUrl("https://hailin.blog/wp-content/uploads/2020/12/software_update.txt")); //包含最新版本软件的下载地址
    quest.setHeader(QNetworkRequest::UserAgentHeader,"RT-Thread ART");
    avatorReply = manager->get(quest);    //发送get网络请求
    connect(avatorReply, SIGNAL(readyRead()), this, SLOT(httpReadyRead()));
    connect(avatorReply, SIGNAL(downloadProgress(qint64, qint64)), this, SLOT(httpDownloadProgress(qint64, qint64)));
    connect(avatorReply, SIGNAL(finished()), this, SLOT(httpFinished()));
}
